...
model = dict(
    ...
    img_view_transformer=dict(
        type="InterDistill_FastBEV_MS_BEV",
        loss_depth_weight=-1,
        grid_config=grid_config,
        data_config=data_config,
        numC_input=256,
        numC_Trans=numC_Trans,
        pct_range=point_cloud_range,
        ...
        is_ms_bev=True,
        num_ms_fusion=1,
        unproject_postion="after",  # before, after
        unproject=True,
        trans_version="v3",
        # When config the following, predefined voxel coordinate settings are performed independently of grid_config.
        z_axis_times=20,  # equal to the third dimension of n_voxels param
        n_voxels=[128, 128, 12],
        ms_bev_fusion=dict(
            in_channel="auto",  # num_ms_fusion * n_voxels[2] * numC_Trans
        ...
        # ),
        # se_like=dict(in_channel=64),
        # bev_channel_reduce=dict(
        #     in_channel=256, out_channel=256, act_cfg=dict(type="ReLU", inplace=True)
        # NOTE self-calibration using for re-encoder
        # selfcalib_conv_config=dict(
        #     type="KD_SCNet", input_channels=64, out_channels=64, num_layers=5
        # ),
        xy_transpose=False,
    ),
...
work_dir = "d3pd/v1/fastbev-swin-all-distill"
