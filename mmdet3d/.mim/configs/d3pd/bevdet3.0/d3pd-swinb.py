...
model = dict(
    ...
    distillation=dict(
        sparse_feats_distill=dict(
            ...
            #     type="Relevance_Distillation", bs=1, bn_dim=512, lambd=0.0051
            # ),
    ...
    pts_bbox_head_stu=dict(
        ...
        ret_det_feats=False,
        task_specific=True,
        in_channels=detFeats_inChannels,
        tasks=[
            dict(num_class=1, class_names=["car"]),
            dict(num_class=2, class_names=["truck", "construction_vehicle"]),
            dict(num_class=2, class_names=["bus", "trailer"]),
...
work_dir = "d3pd/bevdet3.0/swin-samplingdistill-rcdistill-smfd-ema-layer_wise"
