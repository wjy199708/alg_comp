        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.task_specific:
                ...
                for reg_task_id in range(len(name_list)):
                    ...
                    loss_dict[
                        f"%stask{task_id}.loss_%s"
                        % (self.loss_prefix, name_list[reg_task_id])
                    ] = loss_bbox_tmp
            else:
            ...
            loss_dict[f"%stask{task_id}.loss_heatmap" % (self.loss_prefix)] = (
                loss_heatmap
            )
        return loss_dict
            ...
            if isinstance(nms_rescale_factor, list):
                for cid in range(len(nms_rescale_factor)):
                    box_preds[top_labels == cid, 3:6] = (
                        box_preds[top_labels == cid, 3:6] / nms_rescale_factor[cid]
                    )
            else:
                box_preds[:, 3:6] = box_preds[:, 3:6] / nms_rescale_factor
...
@HEADS.register_module()
class My_CenterHead(CenterHead):
    def __init__(self, direct_det=False, ret_det_feats=False, **kwargs):
        super().__init__(**kwargs)
        self.direct_det = direct_det
        self.ret_det_feats = ret_det_feats
