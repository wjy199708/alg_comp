...
class ViewTransformerLiftSplatShoot(BaseModule):
    ...
    def voxel_pooling(self, geom_feats, x):
        ...
        ret_low_level_voxel_feats = final

        # collapse Z
        final = torch.cat(final.unbind(dim=2), 1)

        return final, ret_low_level_voxel_feats
    ...
    def forward(self, input):
        ...
        if self.accelerate:
            ...
            bev_feat, ll_voxel_feats = self.voxel_pooling(geom, volume)

        return bev_feat, depth, ll_voxel_feats
...
class ViewTransformerLSSBEVDepth(ViewTransformerLiftSplatShoot):
    ...
    def forward(self, input):
        """forward computation
        Args:
            input [torch.tensor]: size of (_, _, 512, 1/16, 1/16) or (_, _, 512, 16, 44)
        """
        ...
        if self.accelerate:
            ...
            geom = self.get_geometry(rots, trans, intrins, post_rots, post_trans)
            bev_feat, ret_ll_voxel_feats = self.voxel_pooling(geom, volume)
