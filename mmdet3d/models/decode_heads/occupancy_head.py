...
from .occ_loss import lovasz_softmax, CustomFocalLoss
...
class OccHead(BaseModule):
        ...
        if balance_cls_weight:
            if out_channel == 19:
                self.class_weights = torch.from_numpy(
                    1 / np.log(nusc_class_frequencies[:out_channel] + 0.001)
                )
                self.class_weights = torch.cat([torch.tensor([0]), self.class_weights])
            else:
                if out_channel == 17:
                    nusc_class_frequencies[0] += nusc_class_frequencies[-1]
                self.class_weights = torch.from_numpy(
                    1 / np.log(nusc_class_frequencies[:out_channel] + 0.001)
                )
        else:
            self.class_weights = (
                torch.ones(out_channel) / out_channel
            )  # FIXME hardcode 17
    ...
    def loss_voxel(self, output_voxels, target_voxels, tag):
        ...
        if self.use_dice_loss:
            ...
            loss_dict["loss_voxel_dice_{}".format(tag)] = self.dice_loss(
                visible_pred_voxels, visible_target_voxels
            )
