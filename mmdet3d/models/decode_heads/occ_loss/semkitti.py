    ...
    with autocast(False):
        return (
            F.binary_cross_entropy_with_logits(
                inverse_sigmoid(precision, "A"), torch.ones_like(precision)
            )
            + F.binary_cross_entropy_with_logits(
                inverse_sigmoid(recall, "B"), torch.ones_like(recall)
            )
            + F.binary_cross_entropy_with_logits(
                inverse_sigmoid(spec, "C"), torch.ones_like(spec)
            )
        )
            ...
            if torch.sum(completion_target) > 0:
                ...
                if torch.sum(completion_target) > 0:
                    ...
                    loss_recall = F.binary_cross_entropy_with_logits(
                        inverse_sigmoid(recall, "E"), torch.ones_like(recall)
                    )
