...
@PIPELINES.register_module()
class PhotoMetricDistortionMultiViewImage_DenseRadar(PhotoMetricDistortionMultiViewImage):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
    1. random brightness
...
class GlobalRotScaleTrans(object):
    ...
    def update_transform(self, input_dict):
        ...
        aug_transform = torch.zeros(
            (input_dict["img_inputs"][1].shape[0], 4, 4)
        ).float()
        if "pcd_rotation" in input_dict:
            ...
            aug_transform[:, :3, :3] = (
                torch.eye(3).view(1, 3, 3) * input_dict["pcd_scale_factor"]
            )
        aug_transform[:, :3, -1] = torch.from_numpy(input_dict["pcd_trans"]).reshape(
            1, 3
        )
