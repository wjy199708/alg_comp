...
class PointToMultiViewDepth(object):
    ...
    def __call__(self, results):
        ...
        for cid in range(rots.shape[0]):
            ...
            points_img = (points_lidar.tensor[:, :3] - trans[cid : cid + 1, :]).matmul(
                combine_inv.T
            )
            ...
            if self.sequential:
                ...
                if not type(results["adjacent"]) is list:
                    ...
                    for id in range(len(results["adjacent"])):
                        filename_adjacent = results["adjacent"][id]["cams"][cam][
                            "data_path"
                        ]
                        img_adjacent = Image.open(filename_adjacent)
                        img_adjacent = self.img_transform_core(
        ...
        if self.sequential:
            if self.trans_only:
                if not type(results["adjacent"]) is list:
                    ...
                    if self.aligned:
                        posi_curr = np.array(
                            results["curr"]["ego2global_translation"], dtype=np.float32
                        )
                        posi_adj = np.array(
                            results["adjacent"]["ego2global_translation"],
                            dtype=np.float32,
                        )
                ...
                if not type(results["adjacent"]) is list:
                    ...
                    if self.aligned:
                        ...
                        egocurr2global[:3, 3] = results["curr"][
                            "ego2global_translation"
                        ]

                        egoadj2global = np.eye(4, dtype=np.float32)
                        egoadj2global[:3, :3] = Quaternion(
                            results["adjacent"]["ego2global_rotation"]
                        ).rotation_matrix
                        egoadj2global[:3, 3] = results["adjacent"][
                            "ego2global_translation"
                        ]
...
@PIPELINES.register_module()
class LoadMultiViewImageFromFiles_DenseRadar(LoadMultiViewImageFromFiles_BEVDet):
    def get_lidar2cam_lidar2img(self, rot, tran, intrinsic):
        lidar2cam_r = torch.inverse(rot)
        ...
        return lidar2cam_rt, lidar2img_rt
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam in cams:
            ...
            lidar2cam, lidar2img = self.get_lidar2cam_lidar2img(
                rot=rot, tran=tran, intrinsic=intrin
            )
        ...
        imgs, rots, trans, intrins, post_rots, post_trans, lidar2cams, lidar2imgs = (
            torch.stack(imgs),
            torch.stack(rots),
            torch.stack(trans),
            torch.stack(intrins),
        ...
        return imgs, rots, trans, intrins, post_rots, post_trans, lidar2cams, lidar2imgs

    def __call__(self, results):
        img_inputs = self.get_inputs(results)
        results["img_inputs"] = img_inputs[:-2]
        results["lidar2cams"] = img_inputs[-2]
        results["lidar2imgs"] = img_inputs[-1]
        return results
