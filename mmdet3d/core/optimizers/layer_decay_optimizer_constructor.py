...
def get_layer_id_for_swin(var_name, max_layer_id, backbone_name="img_backbone"):

    if var_name in (f"{backbone_name}.absolute_pos_embed"):
        return 0
    elif var_name.startswith(f"{backbone_name}.patch_embed"):
        return 0
    elif var_name.startswith(f"{backbone_name}.stages"):
        num_layers = [2, 2, 18, 2]
    ...
    def add_params(self, params, module, **kwargs):
        ...
        for name, param in module.named_parameters():
            ...
            if "layer_wise" in decay_type:
                if hasattr(module, "img_backbone"):
                    if "Swin" in module.img_backbone.__class__.__name__:
                    ...
                    if "Swin" in module.backbone.__class__.__name__:
                        layer_id = get_layer_id_for_swin(
                            name,
                            self.paramwise_cfg.get("num_layers"),
